...
equal_assign    :    expr EQ_ASSIGN expr_or_assign              { A.Binop(A.Assign, $1, $3) } (* { $$ = xxbinary($2,$1,$3); } *)
                ;
...
expr    :   INT_CONST           { A.ConstNum (A.Int $1) }(* { $$ = $1;  setId( $$, @$); } *)
    ...
    |   expr COLON  expr        { A.Binop (A.Range, $1, $3) }(* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr PLUS  expr         { A.Binop (A.Plus, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr MINUS expr         { A.Binop (A.Minus, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr MULT expr          { A.Binop (A.Mult, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr DIV expr           { A.Binop (A.Div, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr CARAT expr         { A.Binop (A.Exp, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr USER_OP expr       { A.FuncCall ($2, [$1;$3]) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    (*|   expr '%' expr         { A.Binop (A.Modulus, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *) (* TODO *) *)
    |   expr TILDE expr         { A.Binop (A.Form, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr QUESTION expr      { A.Binop (A.Help, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr LT expr            { A.Binop (A.Lt, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr LE expr            { A.Binop (A.Le, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr EQ expr            { A.Binop (A.Eq, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr NE expr            { A.Binop (A.Neq, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr GE expr            { A.Binop (A.Ge, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr GT expr            { A.Binop (A.Gt, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr AND expr           { A.Binop (A.AndVec, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr OR expr            { A.Binop (A.OrVec, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr AND2 expr          { A.Binop (A.And, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr OR2 expr           { A.Binop (A.Or, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr LEFT_ASSIGN expr   { A.Binop (A.Assign, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr RIGHT_ASSIGN expr  { A.Binop (A.Assign, $3, $1) } (* { $$ = xxbinary($2,$3,$1);  setId( $$, @$); } *)
    |   expr LEFT_SUPER_ASSIGN expr   { A.Binop (A.SuperAssign, $1, $3) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   expr RIGHT_SUPER_ASSIGN expr  { A.Binop (A.SuperAssign, $3, $1) } (* { $$ = xxbinary($2,$3,$1);  setId( $$, @$); } *)
    ...
    |   SYMBOL NS_GET SYMBOL        { A.Binop (A.GetPackage, (A.Ident {A.default_ident with name=$1}), (A.Ident {A.default_ident with name=$3})) } (* { $$ = xxbinary($2,$1,$3);      setId( $$, @$); modif_token( &@1, SYMBOL_PACKAGE ) ; } *)
    |   SYMBOL NS_GET STR_CONST     { A.Binop (A.GetPackage, (A.Ident {A.default_ident with name=$1}), (A.ConstString $3)) } (* { $$ = xxbinary($2,$1,$3);      setId( $$, @$); modif_token( &@1, SYMBOL_PACKAGE ) ; } *)
    |   STR_CONST NS_GET SYMBOL     { A.Binop (A.GetPackage, (A.ConstString $1), (A.Ident {A.default_ident with name=$3})) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   STR_CONST NS_GET STR_CONST  { A.Binop (A.GetPackage, (A.ConstString $1), (A.ConstString $3)) }(* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   SYMBOL NS_GET_INT SYMBOL    { A.Binop (A.GetPackage, (A.Ident {A.default_ident with name=$1}), (A.Ident {A.default_ident with name=$3})) } (* { $$ = xxbinary($2,$1,$3);      setId( $$, @$); modif_token( &@1, SYMBOL_PACKAGE ) ;} *)
    |   SYMBOL NS_GET_INT STR_CONST { A.Binop (A.GetPackage, (A.Ident {A.default_ident with name=$1}), (A.ConstString $3)) } (* { $$ = xxbinary($2,$1,$3);      setId( $$, @$); modif_token( &@1, SYMBOL_PACKAGE ) ;} *)
    |   STR_CONST NS_GET_INT SYMBOL { A.Binop (A.GetPackage, (A.ConstString $1), (A.Ident {A.default_ident with name=$3}))} (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    |   STR_CONST NS_GET_INT STR_CONST  { A.Binop (A.GetPackage, (A.ConstString $1), (A.ConstString $3)) } (* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
    ...
    |   expr AT SYMBOL         { A.Binop (A.ObjAttr, $1, (A.Ident {A.default_ident with name=$3})) }(* { $$ = xxbinary($2,$1,$3);      setId( $$, @$); modif_token( &@3, SLOT ) ; } *)
    |   expr AT STR_CONST      { A.Binop (A.ObjAttr, $1, (A.ConstString $3)) }(* { $$ = xxbinary($2,$1,$3);  setId( $$, @$); } *)
