...
let vec_to_named_sym: smtvar -> S.rvector ->  S.symvec * smtexpr list =
    fun var vec ->
    match vec with
    | S.IntVec i ->
        ...
        ((var, S.RInt), S.NoDepends), [len_constraint] @ gets
    | S.FloatVec f ->
        let gets = Array.to_list (Array.mapi (fun index n ->
            SmtEq (
                smt_getn var index,
        ...
        ((var, S.RFloat), S.NoDepends), [len_constraint] @ gets
    ...
    | S.BoolVec b ->
        ...
        ((var, S.RBool), S.NoDepends), [len_constraint] @ gets 
    | S.SymVec ((_, _), _) -> failwith "Vector is already symbolic"
...
let vec_to_symvec: S.rvector -> S.state -> (S.symvec * S.state) =
    ...
    let sy, pc = vec_to_named_sym name vec in
    let state'' = S.state_add_pathcons_list pc state' in
    (sy, state'')
...
let sym_op: (smtvar -> S.symvec list -> S.symdef * smtexpr list) ->
    ...
    let newvec_def, newvec_pcs = smt_op newvec_name sym_args in
    let state''' = S.state_add_pathcons_list newvec_pcs state'' in
    (S.SymVec (newvec_def, depends), state''')

(* Produces an smt_op compatible with symbolize for a function with two arguments *)
let arg2listize: (smtvar -> S.symvec -> S.symvec -> S.symdef * smtexpr list) ->
        smtvar -> S.symvec list -> S.symdef * smtexpr list =
    fun f name args ->
    match args with
...
let arg3listize: (smtvar -> S.symvec -> S.symvec -> S.symvec -> S.symdef * smtexpr list) ->
        smtvar -> S.symvec list -> S.symdef * smtexpr list =
    fun f name args ->
    match args with
...
let symbolic_sum: smtvar -> S.symvec -> S.symdef * smtexpr list =
    fun new_name ((name, ty),_) ->
...
let symbolic_concat: smtvar -> S.symvec -> S.symvec -> S.symdef * smtexpr list =
    ...
    (new_name, new_ty), [len; a1_elts; a2_elts]

(* Produces a definition for symbolic x[y]. Assumes y is an index vector for x. *)
let symbolic_subscript: smtvar -> S.symvec -> S.symvec -> S.symdef * smtexpr list =
    fun new_name ((name1, ty1), _) ((name2, ty2), _) ->
    ...
    (new_name, ty1), [len;value;name2_c]

(* Produces a definition for symbolic x[y] <- z. Assumes y is a proper index vector for x, and
 that z has length 1. *)
let symbolic_subscript_assign: smtvar -> S.symvec -> S.symvec -> S.symvec -> S.symdef * smtexpr list =
    fun new_name ((name1, ty1), _) ((name2, ty2), _) ((name3, ty3), _) ->
    let _ = match ty2 with
    | S.RInt -> ()
    | _ -> failwith "Subscript with non-integer symbolic vector!" in
    ...
    (new_name, new_ty), [len;vals;name2_c;name3_c]
...
let symbolic_op: smtvar -> (smtexpr -> smtexpr -> smtexpr) ->
        ...
        S.symvec -> S.symvec -> S.symdef * smtexpr list =
    fun new_name smt_combo combo_assume ((name1, ty1), _) ((name2, ty2), _) ->
    let new_ty = match_tys ty1 ty2 in
...
(* Symbolic comparison is the same as any other binop, but the result is a symbolic
  bool vector. *)
let symbolic_cmp: smtvar -> (smtexpr -> smtexpr -> smtexpr) ->
        S.symvec -> S.symvec -> S.symdef * smtexpr list =
