...
let bop_rvectors: (S.rint -> S.rint -> S.rint) ->
    ...
    | (_, _) -> let partial_op = fun name sy1 sy2 -> Sym.symbolic_op name fsy sym_assume sy1 sy2 in
        Sym.sym_op (Sym.arg2listize partial_op) [lhs;rhs] state
...
let cmp_rvectors: (S.rint -> S.rint -> S.rbool) ->
    ...
    | (S.BoolVec l, S.BoolVec r) -> (S.BoolVec (C.array_map2 fb l r), state)
    | (_, _) -> let partial_op = fun name sy1 sy2 -> Sym.symbolic_cmp name fsy sy1 sy2 in
        Sym.sym_op (Sym.arg2listize partial_op) [lhs;rhs] state
    | _ -> failwith "Can't bop vectors with incompatible types"
...
let logic_vec_rvectors: (S.rbool -> S.rbool -> S.rbool) ->
    ...
    | (S.BoolVec l, S.BoolVec r) -> (S.BoolVec (C.array_map2 fb l r), state)
    | (_, _) -> let partial_op = fun name sy1 sy2 -> Sym.symbolic_logic_vec_op name fsy symfail sy1 sy2 in
        Sym.sym_op (Sym.arg2listize partial_op) [lhs;rhs] state
    | _ -> failwith "Can't bop vectors with incompatible types"
...
let logic_single_rvectors: (S.rbool -> S.rbool -> S.rbool) -> 
    ...
    | (S.BoolVec l, S.BoolVec r) -> 
        ...
        v.(0) <- fb l.(0) r.(0);
        (S.BoolVec v, state)
    | (_, _) -> let partial_op = fun name sy1 sy2 -> Sym.symbolic_logic_single_op name fsy symfail sy1 sy2 in
        Sym.sym_op (Sym.arg2listize partial_op) [lhs;rhs] state
    | _ -> failwith "Can't bop vectors with incompatible types"
